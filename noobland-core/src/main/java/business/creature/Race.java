...
public enum Race {
	...
	public static Race getRaceByName(String name) {
		if ("Human".equalsIgnoreCase(name)) return Race.HUMAN;
		if ("Mountain Dwarf".equalsIgnoreCase(name)) return Race.MOUNTAIN_DWARF;
		if ("High Elf".equalsIgnoreCase(name)) return Race.HIGH_ELF;
		if ("Wood Elf".equalsIgnoreCase(name)) return Race.WOOD_ELF;
		if ("Lightfoor Halfling".equalsIgnoreCase(name)) return Race.LIGHTFOOT_HALFLING;
		return null;
	}
